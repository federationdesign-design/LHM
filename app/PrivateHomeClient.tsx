 ...
 *
 * Sets style.viewTransitionName = 'hero-image' on the desktop hero
 * so the browser morphs from the splash panel's matching image when
 * navigating from / to /private. The mobile hero uses a different
 * image file so isn't named — the browser falls back to its default
 * crossfade for mobile, which is a fine compromise.
 *
 * The transition is invoked from SplashClient via
 * document.startViewTransition(); this file only needs to declare
 * which element should be the morph target.
 */
export default function PrivateHomeClient() {
    ...
    <>
          ...
          '@graph': [
            {
              ...
              sameAs: ['https://www.tripadvisor.com', 'https://www.linkedin.com/in/lucy-hall-massage-47369141/'],
            },
          ],
        }) }}
      />
      ...
      <main style={{ background: '#000000' }}>
        ...
        <div ref={heroRef} className={styles.hero} style={{ height: '100vh', minHeight: '100vh' }}>
          ...
          {/* Desktop hero — same file as the splash's private panel.
              Carries viewTransitionName='hero-image' matching the
              splash. The browser sees the same name on both pages
              and morphs the box position/size during navigation
